		...
		if not email:
			...
			if form.password.data == form.confirm_password.data:
				...
				new_user = User(username=form.username.data, 
								...
								sex=form.radios.data, 
								appeal=form.selects.data, 
								about=form.textarea.data)
	...
	if view_form == 'sign_in':
		...
		if form_login.validate_on_submit():
			if login():
				...
				view_form='sign_in', slice1=slice1, slice2=slice2))

		return render_template('index.html',view_form=view_form,
											weather=weather,
											time=time,
		...
		if form_signup.validate_on_submit():
			if register():
				return redirect(url_for('index', view_form='sign_in',
												slice1=slice1, 
												slice2=slice2))
...
def post(post_id, view_form):
	...
	if view_form == 'sign_in':
		...
		return render_template('post.html', view_form=view_form,
											form_login=form_login,
											posts=posts,
											post=post,
											user_posts=user_posts,
...
def user_posts(slice1, slice2):
	...
	time = datetime.now()
	view_posts = Post.query.filter_by(
		post_author=current_user.username).order_by(Post.date_posted.desc()).slice(slice1,slice2)
	user_posts = check_login_for_posts_length()
...
def add_post(slice1, slice2):
	...
	if form_blog.validate_on_submit():
		...
		new_post = Post(
			title=form_blog.title.data,subtitle=form_blog.subtitle.data,
			content=form_blog.content.data,date_posted=datetime.now(),
			post_author=current_user.username)
		db.session.add(new_post)
		db.session.commit()
		return redirect(url_for('index'))

	return render_template('add_post.html', slice1=slice1,
											slice2=slice2,
											weather=weather,
											time=time,
											form_blog=form_blog,
